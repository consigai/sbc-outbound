...
const sipStatusCodes = require('sip-status');

/**
 * Map of SIP 6xx class codes that are valid per RFC but missing from the
 * `sip-status` module.  We map each to the closest standard code so that
 * drachtio's Response#send (which checks `status in sip-status`) does not throw.
 */
const NONSTANDARD_STATUS_MAP = {
  607: 603,   // Unwanted  → Decline
  608: 603,   // Rejected  → Decline
};

/**
 * Returns a status that will pass Response#send validation:
 *   typeof status === 'number' && status in require('sip-status')
 *
 * If the status is already known to sip-status, return it unchanged.
 * If it is a number we recognise (e.g. 608), map to the nearest standard code.
 * Otherwise fall back to 500.
 *
 * @param {*} status - value passed to res.send(status, ...)
 * @returns {number} a status code guaranteed to be in sip-status
 */
const normalizeSipStatus = (status) => {
  if (typeof status === 'number' && status in sipStatusCodes) return status;
  if (typeof status === 'number' && status in NONSTANDARD_STATUS_MAP) return NONSTANDARD_STATUS_MAP[status];
  if (typeof status === 'number' && status >= 100 && status <= 699) return 500;
  return 500;
};

/**
 * Monkey-patches res.send so it never receives a status code that is missing
 * from the sip-status module.  This prevents the crash in drachtio-srf when a
 * carrier returns a non-standard SIP code (e.g. 608 Rejected) that is not in
 * the sip-status lookup table.
 *
 * We patch in-place rather than creating a wrapper object so that the `finished`
 * / `finalResponseSent` state stays accurate on the original res.
 *
 * @param {object} res - drachtio response object
 * @param {object} logger - pino/bunyan logger
 */
const patchResWithSafeSend = (res, logger) => {
  const originalSend = res.send;
  res.send = function(status, ...rest) {
    const safe = normalizeSipStatus(status);
    if (safe !== status) {
      logger.warn({ rawStatus: status, normalized: safe },
        'res.send called with SIP status not in sip-status module; normalizing to avoid crash');
    }
    return originalSend.call(this, safe, ...rest);
  };
};
...
class CallSession extends Emitter {
  ...
  async connect() {
    ...
    // Initialize rtpEngine options now that teams is defined
    ...
    // Now create rtpEngineResource after del is assigned
    this.rtpEngineResource = {destroy: this.del.bind(null, this.rtpEngineOpts.common)};
    
    // Log the initial rtpengine configuration for this call session
    this.logger.info({
      operation: 'call_session_init',
      callId: this.req.get('Call-ID'),
      initialRtpEngineOpts: this.rtpEngineOpts,
      useWss: this.useWss,
      teams,
      mediaPath: this._mediaPath
    }, 'CallSession initialized with rtpengine options');
    ...
    try {
          ...
          if (gws.length) {
            ...
            gws.forEach((o) => {
              ...
              if (o.protocol === 'tls/srtp') {
                ...
                // Log the updated rtpengine options for encrypted media
                this.logger.info({
                  operation: 'encrypted_media_config_update',
                  callId: this.req.get('Call-ID'),
                  updatedRtpEngineOpts: this.rtpEngineOpts,
                  padCrypto: o.pad_crypto,
                  gateway: o
                }, 'Updated rtpengine options for encrypted media (SRTP)');
      ...
      // Log the initial offer options for troubleshooting
      this.logger.info({
        operation: 'initial_offer',
        callId: this.req.get('Call-ID'),
        rtpEngineOpts: opts,
        mediaPath: this._mediaPath,
        privateNetworkTargets: toPrivate,
        publicNetworkTargets: toPublic
      }, 'Sending initial offer to rtpengine');
      ...
      while (uris.length) {
        ...
        patchResWithSafeSend(this.res, this.logger);
        try {
          const {uas, uac} = await this.srf.createB2BUA(this.req, this.res, uri, {
            ...
            localSdpA: async(sdp, res) => {
              ...
              // Log the answer options for troubleshooting
              this.logger.info({
                operation: 'answer',
                callId: this.req.get('Call-ID'),
                rtpEngineOpts: opts,
                mediaPath: this._mediaPath,
                uacTag: this.rtpEngineOpts.uac.tag,
                uasTag: this.rtpEngineOpts.uas.tag
              }, 'Sending answer to rtpengine');
  ...
  _setHandlers({uas, uac}) {
    ...
    [uas, uac].forEach((dlg) => {
      dlg.on('destroy', async(bye) => {
        ...
        // Log rtpengine resource cleanup
        this.logger.info({
          operation: 'rtpengine_cleanup',
          callId: this.req.get('Call-ID'),
          mediaPath: this._mediaPath,
          dialogType: dlg.type
        }, 'Destroying rtpengine resource for call cleanup');
...
Duration=${payload.duration} `
  ...
  async _onReinvite(dlg, req, res) {
    try {
      ...
      // Log the reinvite offer options for troubleshooting
      this.logger.info({
        operation: 'reinvite_offer',
        callId: this.req.get('Call-ID'),
        rtpEngineOpts: opts,
        reason,
        isReleasingMedia,
        ...
        mediaPath: this._mediaPath,
        dialogType: dlg.type
      }, 'Sending reinvite offer to rtpengine');
      ...
      // Log the reinvite answer options for troubleshooting
      this.logger.info({
        operation: 'reinvite_answer',
        callId: this.req.get('Call-ID'),
        rtpEngineOpts: opts,
        reason,
        isReleasingMedia,
        mediaPath: this._mediaPath,
        dialogType: dlg.type
      }, 'Sending reinvite answer to rtpengine');
    ...
    try {
      ...
      if (dlg.type === 'uas' && req.has('X-Reason')) {
        ...
        if (reason.startsWith('mute')) {
          ...
          // Log the playDTMF options for troubleshooting
          this.logger.info({
            operation: 'play_dtmf',
            callId: this.req.get('Call-ID'),
            rtpEngineOpts: dtmfOpts,
            dtmfCode: code,
            dtmfDuration: duration
          }, 'Sending playDTMF to rtpengine');
        ...
        if (this.isMediaReleased) {
          ...
          // Log the playDTMF options for troubleshooting
          this.logger.info({
            operation: 'sip_info_dtmf_to_rfc2833',
            callId: this.req.get('Call-ID'),
            rtpEngineOpts: opts,
            dtmfCode: code,
            dtmfDuration: duration
          }, 'Converting SIP INFO DTMF to RFC 2833 via rtpengine');
