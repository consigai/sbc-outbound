...
function makeRtpEngineOpts(req, srcIsUsingSrtp, dstIsUsingSrtp, padCrypto, teams) {
  // Reload transcoding configs on each use for dynamic configuration
  const rtpConfigPath = path.join(__dirname, '../data/rtp-transcoding.json');
  const srtpConfigPath = path.join(__dirname, '../data/srtp-transcoding.json');
  
  // Clear require cache to force reload
  delete require.cache[require.resolve(rtpConfigPath)];
  delete require.cache[require.resolve(srtpConfigPath)];
  
  const rtpCharacteristics = require(rtpConfigPath);
  const srtpCharacteristics = require(srtpConfigPath);
  ...
  // Log the final resolved rtpengine options for troubleshooting
  ...
  if (logger) {
    logger.info({
      callId,
      rtpEngineOpts: finalOpts,
      configParams: {
        srcIsUsingSrtp,
        dstIsUsingSrtp,
        padCrypto,
        teams,
        recordAllCalls: process.env.JAMBONES_RECORD_ALL_CALLS,
        codecOfferWithOrder: process.env.JAMBONES_CODEC_OFFER_WITH_ORDER
      },
      loadedConfigs: {
        rtpCharacteristics,
        srtpCharacteristics
      }
    }, 'Final resolved rtpengine options for call');
  }
  else {
    // Fallback logging when logger is not available
    console.log(`[${callId}] Final resolved rtpengine options:`, JSON.stringify({
      rtpEngineOpts: finalOpts,
      configParams: {
        srcIsUsingSrtp,
        dstIsUsingSrtp,
